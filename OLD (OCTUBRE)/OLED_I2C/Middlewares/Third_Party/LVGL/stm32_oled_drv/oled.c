...
static uint8_t send_data(lv_disp_drv_t *disp_drv, void *bytes, size_t bytes_len){
	...
	I2C_MasterSendData(OLED_I2C, OLED_I2C_ADDRESS, data , bytes_len);
	return 0;
}
static uint8_t send_pixels(lv_disp_drv_t *disp_drv, void *color_buffer, size_t buffer_len){
	...
	while((OLED_I2C->SR2 & I2C_SR2_BUSY));
	...
	SlaveAddr =  OLED_I2C_ADDRESS<<1 ;				//A7-A1 (R/W)
	SlaveAddr &=~(1U);						//write operation
	OLED_I2C->DR = SlaveAddr;
	...
	tmp = OLED_I2C->SR2;
	(void)tmp;
	//se envia el primer byte
	while(!(OLED_I2C->SR1 & I2C_SR1_TXE));
	OLED_I2C->DR = OLED_CONTROL_BYTE_DATA_STREAM;
	//se envia el resto de la trama
	while(len > 0){
...
#if (SSD1306_DISPLAY_ORIENTATION == DISP_ROT_Landscape)
	orientation_1 = OLED_CMD_SET_SEGMENT_REMAP;
	orientation_2 = OLED_CMD_SET_COM_SCAN_MODE_REMAP;
#elif(SSD1306_DISPLAY_ORIENTATION == DISP_ROT_Inv_Landscape)
	orientation_1 = 0xA0;
	orientation_2 = OLED_CMD_SET_COM_SCAN_MODE_NORMAL;
#else
#error "orientacion no soportada"
#endif
