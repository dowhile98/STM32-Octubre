...
int main(void)
{
  ...
  MX_GPIO_Init();
  /* USER CODE BEGIN 2 */
  //ITM_Printf("hola mundo desde ITM\r\n");
  uint32_t count = 0, n;
  float res;
  ...
  while (1)
  {
    ...
    count += 10;
    res = 10 * sin((double) count * M_PI / 180) + 10;
    ITM_SendValue(2, (uint32_t)res);
    res = 10 * sin((double) count * M_PI /45 ) + 10;
    ITM_SendValue(3, (uint32_t)res);
    /* USER CODE END WHILE */
